...
import { formatPhotographyPrice, photographyPackages } from '@/lib/photographyCatalog';
...
export default function ServiceAddedConfirmation() {
  ...
  const service = eventServices.find((item) => item.id === `photography-${selectedPackage.id}`);
  const serviceTitle = packageName || service?.packageName || selectedPackage.title;
  const servicePrice = service?.price ?? selectedPackage.price;
    ...
    <SafeAreaView style={styles.screen} edges={['top']}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={[styles.content, { paddingBottom: Math.max(insets.bottom, 0) + 22 }]}>
        ...
        <View style={styles.successWrap}>
          ...
          <View style={styles.successCircle}><CircleCheck size={72} color="#2cc56f" fill="#ffffff" strokeWidth={1.8} /></View>
        </View>
        ...
        <Card style={styles.addedCard} className="border-[#e7eaf0] bg-white shadow-sm shadow-black/5">
          <CardContent style={styles.addedContent}>
            ...
            <View style={styles.serviceRow}>
              <Image source={selectedPackage.image} resizeMode="cover" style={styles.serviceImage} />
              <View style={styles.serviceCopy}><Text style={styles.serviceTitle}>{serviceTitle}</Text><View style={styles.rating}><Star size={11} color="#ffb900" fill="#ffb900" /><Text style={styles.ratingText}>4.8 (128 reviews) • 150+ Booked</Text></View><Text style={styles.serviceDescription}>Full day coverage with candid,{`\n`}posed shots and premium editing.</Text></View>
            </View>
        ...
        <Card style={styles.summaryCard} className="border-[#e7eaf0] bg-white shadow-none"><CardContent style={styles.summaryContent}><Text style={styles.summaryTitle}>Total Summary</Text><View style={styles.summaryRow}><Text style={styles.summaryText}>{serviceTitle}</Text><Text style={styles.summaryText}>{formatPhotographyPrice(servicePrice)}</Text></View><View style={styles.summaryDivider} /><View style={styles.summaryRow}><Text style={styles.totalText}>Total Amount</Text><Text style={styles.totalText}>{formatPhotographyPrice(servicePrice)}</Text></View></CardContent></Card>

        <View style={styles.actions}><Button variant="outline" className="h-12 rounded-md border-[#ff5a2a] bg-white" onPress={() => router.replace('/EventServices')}><Text className="text-[16px] font-bold text-[#ff5a2a]">Add More Services</Text></Button><Button className="mt-3 h-12 rounded-md bg-[#ff8f42]" onPress={() => router.navigate('/Cart')}><ShoppingCart size={18} color="#ffffff" /><Text className="text-[16px] font-bold text-white">View Cart</Text></Button></View>
      </ScrollView>
    </SafeAreaView>
  );
}
...
const styles = StyleSheet.create({
  ...
  summaryText: { color: '#5d687b', fontSize: 15, lineHeight: 19 },
